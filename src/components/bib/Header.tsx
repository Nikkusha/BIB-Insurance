...
export default function Header() {
  ...
  const DropdownMenu = ({ label, items, id }: { label: string; items: { label: string; slug: string }[]; id: string }) => (
    ...
    >
      <button
        className="flex items-center gap-1 text-sm font-medium text-primary-foreground hover:text-primary-foreground/70 transition-colors py-2"
  ...
  return (
    <>
      <header className="sticky top-0 z-40 bg-primary border-b border-primary-foreground/10 h-20">
        <div className="container-bib flex items-center justify-between h-full">
          {/* Logo */}
          <Link to={`/?lang=${lang}`}>
            <img src="/bib-logo.png" alt="BIB Logo" className="h-10 w-auto brightness-0 invert" />
          </Link>
          ...
          <div className="hidden md:flex items-center gap-3">
            <button
              onClick={toggleLang}
              className="rounded-md border border-primary-foreground/30 px-3 py-1.5 text-xs font-semibold text-primary-foreground hover:bg-primary-foreground/10 transition-colors"
            >
              {d.nav.langToggle}
            </button>
            <button
        ...
        {/* Mobile menu */}
        {mobileOpen && (
          <div className="md:hidden border-t border-primary-foreground/10 bg-primary animate-fade-in">
            <div className="container-bib py-4 space-y-3">
              <p className="text-xs font-semibold text-primary-foreground/50 uppercase tracking-wider">{d.nav.company}</p>
              {d.companyDropdown.map((item) => (
                <Link key={item.slug} to={navLink(item.slug)} onClick={() => setMobileOpen(false)} className="block pl-2 text-sm text-primary-foreground hover:text-primary-foreground/70 py-1">
                  {item.label}
                </Link>
              ))}
              <p className="text-xs font-semibold text-primary-foreground/50 uppercase tracking-wider pt-2">{d.nav.products}</p>
